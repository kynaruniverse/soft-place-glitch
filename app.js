 ...
 * MAKÉ — app.js (V6)
 *
 * This file is now a thin orchestrator.  All feature code lives in focused
 * modules that can be found, tested, and edited independently:
 *
 *   core/schema.js        — data shape, createItem, extendItem
 *   core/state.js         — reactive state store (V3: filterFavourites persisted)
 *   core/storage.js       — IndexedDB R/W (V4: localStorage fallback)
 *
 *   utils/helpers.js      — esc, parseTags, relativeDate, showToast, icons
 *   utils/rich-text.js    — execCommand wrapper (isolated for future migration)
 ...
 *
 *   ui/cards.js           — background-layer card grid
 *   ui/stickies.js        — floating sticky layer (V2: cleanup leak fixed)
 *   ui/note-editor.js     — full-page rich-text editor (live tag preview)
 *   ui/code-editor.js     — full-page code editor (syntax highlighting)
 *   ui/modals.js          — modal factory, link / sticky / settings / context
 *
 *   features/ambient.js   — time-of-day sort
 *   features/data.js      — export / import (V2: dedup on import)
 *   features/drawer.js    — side drawer (V2: fav filter persisted)
...
async function init() {
  ...
  // Warn the user (once) if we fell back to localStorage.
  if (isUsingFallback()) {
...
function _attachShellListeners() {
  // Tab navigation + FAB + outside-click to close add-menu.
  ...
  // Update the date widget every minute.
  setInterval(() => {
    const { day, date } = _getLiveDate();
